...
pub async fn encrypt_files(input: EncryptDataInput, _window: Window) -> CommandResponse<String> {
    ...
    let service = crate::services::crypto::EncryptionService::new();

    match service.encrypt_files(input).await {
        Ok(encrypted_path) => Ok(encrypted_path),
        Err(crypto_error) => {
            // Convert service error to command error
            Err(Box::new(crate::commands::types::CommandError::operation(
                crate::commands::types::ErrorCode::EncryptionFailed,
