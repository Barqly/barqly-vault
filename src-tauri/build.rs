...
fn main() {
    // Generate TypeScript definitions if the feature is enabled
    if cfg!(feature = "generate-types") {
        generate_typescript_definitions();
    }

    // Re-run if any of our source files change
    println!("cargo:rerun-if-changed=src/commands/types.rs");
...
/// Generate TypeScript definitions from Rust types
fn generate_typescript_definitions() {
    ...
    let typescript_content = generate_typescript_content();
    let typescript_path = generated_dir.join("types.ts");
