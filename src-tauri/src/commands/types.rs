...
impl ProgressManager {
    ...
    fn should_emit_immediately(&self, current_progress: f32) -> bool {
        use crate::constants::{PROGRESS_FORCE_EMIT_THRESHOLD, PROGRESS_IMMEDIATE_EMIT_VALUES};
    ...
    fn try_emit_pending_update(&mut self) {
        use crate::constants::PROGRESS_DEBOUNCE_INTERVAL_MS;
...
#[cfg(test)]
mod debounce_tests {
    //! Unit tests for progress debouncing functionality
    //!
    //! These tests validate the debouncing implementation in ProgressManager
    //! to ensure it reduces IPC calls while maintaining smooth progress updates.

    use super::{ProgressManager, ProgressUpdate};
