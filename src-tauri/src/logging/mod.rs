...
use crate::services::shared::infrastructure::path_management::PathProvider;
...
/// Get the platform-specific log directory using PathProvider
/// This ensures consistency with the rest of the application
fn get_log_dir() -> Result<PathBuf, io::Error> {
    // Use the centralized PathProvider
    let provider = PathProvider::global().map_err(|e| {
        io::Error::new(
            io::ErrorKind::NotFound,
            format!("PathProvider not initialized: {}", e),
        )
    })?;

    let provider = provider
        .read()
        .map_err(|_| io::Error::other("PathProvider lock poisoned"))?;

    let log_dir = provider.logs_dir().map_err(|e| {
        io::Error::new(
            io::ErrorKind::NotFound,
    ...
    provider.ensure_dir_exists(&log_dir).map_err(|e| {
        io::Error::new(
