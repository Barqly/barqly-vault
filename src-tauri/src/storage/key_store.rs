...
mod tests {
    ...
    fn create_test_env() -> Result<TempDir, StorageError> {
        let temp_dir = TempDir::new().map_err(|e| StorageError::IoError(e))?;

        // Create a mock keys directory structure
        let keys_dir = temp_dir.path().join("keys");
        fs::create_dir_all(&keys_dir).map_err(|e| StorageError::IoError(e))?;
    ...
    fn test_save_and_load_key() {
        ...
        // The actual save/load functionality is tested in integration tests

        let label = "test-key";

        // Test that key_exists returns false for non-existent keys
        // Note: This will fail in test context due to directories crate behavior
        // In a real test environment, we'd mock the directories crate
        let result = key_exists(label);
        // The result depends on the test environment, so we just check it doesn't panic
    ...
    fn test_key_info_access_tracking() {
        let mut key_info =
            KeyInfo::new("test-label".to_string(), PathBuf::from("/test/path"), None);
