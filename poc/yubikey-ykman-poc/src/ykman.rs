...
pub fn check_ykman() -> Result<Option<String>> {
    ...
    let output = Command::new(&ykman_path).arg("--version").output();
...
pub fn get_yubikey_serial() -> Result<String> {
    ...
    let output = Command::new(get_ykman_path()).arg("list").output()?;
    ...
    Err(YubiKeyError::OperationFailed(
        "Could not parse serial from ykman list".to_string(),
    ))
}
...
pub fn check_age_plugin() -> Result<Option<String>> {
    ...
    let output = Command::new(&age_plugin_path).arg("--version").output();
...
fn parse_piv_info(info: &str) -> Result<YubiKeyInfo> {
    ...
    for line in info.lines() {
        ...
        } else if line.contains("Using default Management key")
            || line.contains("Management key is the default")
        {
...
pub fn change_pin(old_pin: &str, new_pin: &str) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        debug!("PIN change failed - stderr: {stderr}, stdout: {stdout}");
