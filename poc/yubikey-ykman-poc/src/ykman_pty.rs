...
pub fn set_management_key_protected_pty(pin: &str, current_key: Option<&str>) -> Result<()> {
    ...
    loop {
        if start.elapsed() > OPERATION_TIMEOUT {
            ...
            return Err(YubiKeyError::OperationFailed(
                "Management key change timed out".to_string(),
            ));
        }
        ...
        match reader.read(&mut byte_buffer) {
                ...
                match child.try_wait() {
                    Ok(Some(status)) => {
                        if status.success() {
                            ...
                            return Err(YubiKeyError::ManagementKeyError(format!(
                                "Failed to set management key. Output: {output_buffer}"
                            )));
                        }
                    }
                    Ok(None) => {
            ...
            Ok(n) => {
                ...
                if !pin_sent
                    && (output_buffer.contains("Enter PIN")
    ...
    match child.wait() {
        ...
        _ => Err(YubiKeyError::ManagementKeyError(format!(
            "Failed to set management key. Final output: {output_buffer}"
        ))),
    }
}
