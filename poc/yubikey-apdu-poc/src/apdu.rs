...
pub fn set_pin_protected_management_key(
    pin: &str,
    touch_required: bool,
) -> Result<()> {
    ...
    // Connect to the card
    let card = ctx.connect(reader, pcsc::ShareMode::Shared, Protocols::T1)
    ...
    match authenticate_with_default_key(&card) {
        ...
        Err(e) => {
            warn!("Authentication failed: {}. Trying to proceed without auth (common after reset)", e);
...
fn select_piv_application(card: &Card) -> Result<()> {
    ...
    let response = card.transmit(&apdu, &mut response)
...
fn authenticate_with_default_key(card: &Card) -> Result<()> {
    ...
    match authenticate_with_algorithm(card, ALGO_AES192) {
        ...
        Err(e) => {
            debug!("AES-192 authentication failed: {}, trying TDES", e);
...
fn authenticate_with_algorithm(card: &Card, algo: u8) -> Result<()> {
    debug!("Authenticating with algorithm 0x{:02X}", algo);
    ...
    debug!("Sending witness request APDU: {}", hex::encode(&get_witness));
    
    let mut response = [0; MAX_BUFFER_SIZE];
    let response = card.transmit(&get_witness, &mut response)
        .context("Failed to get witness")?;
    
    // Debug: Log the raw response
    debug!("Raw witness response: {}", hex::encode(&response[..response.len().min(32)]));
    ...
    debug!("Got challenge ({} bytes): {}", challenge_len, hex::encode(&challenge[..challenge_len]));
    ...
    debug!("Encrypted response ({} bytes): {}", response_len, hex::encode(&encrypted_response));
    ...
    debug!("Sending authentication response: {}", hex::encode(&send_auth));
    
    let mut response = [0; MAX_BUFFER_SIZE];
    let response = card.transmit(&send_auth, &mut response)
...
fn extract_witness_full(response: &[u8]) -> Result<([u8; 16], usize)> {
    ...
    for i in 0..response.len().saturating_sub(4) {
        if response[i] == TAG_AUTH_WITNESS || response[i] == TAG_AUTH_CHALLENGE {
            ...
            if i + 2 + len <= response.len() {
                ...
                debug!("Found witness/challenge with tag 0x{:02X} at offset {}, length {}", response[i], i, len);
    ...
    debug!("Response bytes: {}", hex::encode(&response[..response.len().min(32)]));
    
    Err(anyhow::anyhow!("Failed to extract witness from response - tag not found"))
}
...
fn extract_response(response: &[u8]) -> Result<[u8; 8]> {
    ...
    Err(anyhow::anyhow!("Failed to extract response from authentication"))
}
...
fn encrypt_aes192_full(data: &[u8], key: &[u8]) -> Result<[u8; 16]> {
    ...
    if key.len() != 24 {
        return Err(anyhow::anyhow!("AES-192 requires 24-byte key, got {}", key.len()));
    }
    
    // Data must be exactly 16 bytes for AES
    if data.len() != 16 {
        return Err(anyhow::anyhow!("AES-192 requires 16-byte data block, got {}", data.len()));
    }
...
fn store_protection_metadata(card: &Card) -> Result<()> {
    ...
    let key = &MGMT_KEY_STORAGE.lock().unwrap().clone().unwrap_or([0u8; 24]);
    ...
    if response.len() >= 2 {
        ...
        if sw1 != 0x90 || sw2 != 0x00 {
            debug!("Protection metadata storage returned: {:02X}{:02X}", sw1, sw2);
        }
    }
...
fn check_response_status(response: &[u8], operation: &str) -> Result<()> {
    ...
    match (sw1, sw2) {
        ...
        (0x63, _) => {
            ...
            Err(anyhow::anyhow!("{}: Wrong PIN, {} tries left", operation, tries_left))
        }
        (0x69, 0x82) => Err(anyhow::anyhow!("{}: Security condition not satisfied", operation)),
        (0x69, 0x83) => Err(anyhow::anyhow!("{}: Authentication method blocked", operation)),
        ...
        _ => Err(anyhow::anyhow!("{} failed: {:02X}{:02X}", operation, sw1, sw2))
    }
}
