...
pub use hybrid::{complete_yubikey_setup, initialize_yubikey_hybrid, TouchPolicy};
...
impl ProtectedManagementKey {
    ...
    pub fn set_on_yubikey(&self, pin: &str) -> Result<()> {
        ...
        // Open YubiKey
        let mut yubikey = YubiKey::open().context("Failed to open YubiKey")?;
...
pub fn initialize_yubikey_with_protected_key(pin: &str, touch_policy: TouchPolicy) -> Result<()> {
    info!("Initializing YubiKey with PIN-protected TDES management key");

    // Find YubiKey
    let yubikey = YubiKey::open().context("Failed to open YubiKey")?;

    info!("YubiKey found: {:?}", yubikey.serial());

    // Generate random protected management key
    let mgmt_key =
        ProtectedManagementKey::generate_random(MgmKeyProtection::Protected, touch_policy);
