...
fn ensure_protected_tdes_management_key(pin: &str) -> Result<()> {
    ...
    let mut child = cmd.spawn().context("Failed to spawn ykman")?;

    if let Some(mut stdin) = child.stdin.take() {
        use std::io::Write;
        writeln!(stdin, "{pin}").context("Failed to write PIN to ykman")?;
    }

    let status = child.wait().context("Failed to wait for ykman")?;

    if !status.success() {
        return Err(anyhow::anyhow!(
            "ykman failed to set protected TDES management key"
        ));
    }
...
pub fn generate_age_identity(
    ...
    recipient
        .ok_or_else(|| anyhow::anyhow!("Failed to generate age recipient - no output received"))
}
...
fn change_pin_with_crate(old_pin: &str, new_pin: &str) -> Result<()> {
    let mut yk = YubiKey::open().context("Failed to open YubiKey")?;
...
fn change_puk_with_crate(old_puk: &str, new_puk: &str) -> Result<()> {
    ...
    match yk.change_puk(old_puk.as_bytes(), new_puk.as_bytes()) {
        Ok(_) => {
            info!("✅ PUK changed successfully from {old_puk} to {new_puk}");
            Ok(())
        }
        Err(e) => {
