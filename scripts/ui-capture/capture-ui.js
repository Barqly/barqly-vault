...
class UICaptureTool {
  ...
  async initialize() {
    try {
      ...
      await fs.mkdir(path.join(this.sessionDir, "screenshots"), {
        recursive: true,
      });
  ...
  async startCaptureSession() {
    ...
    console.log(
      "- Press 'c' + Enter for active window capture with 3-second delay",
    );
  ...
  async captureCurrentScreen(delaySeconds = 3) {
    ...
    try {
      ...
      if (
        ...
        console.log(
          "   Try running: cd scripts/ui-capture && npm install --force",
        );
      }
  ...
  async macOSActiveWindowCapture() {
    ...
    try {
      ...
      console.log(
        "ℹ️  Native window capture failed, using desktop capture fallback...",
      );
  ...
  async testScreenshot() {
    ...
    try {
      ...
      console.log(
        "The tool should work correctly. Try capturing with 'c' command.",
      );
      ...
      if (error.message.includes("permission") || error.code === "EACCES") {
        ...
        console.log(
          "  - Try running with: sudo node capture-ui.js (not recommended)",
        );
      }
      console.log("");
    }
  ...
  async finalizeCaptureSession() {
    ...
    if (generateAnalysis) {
      console.log(
        `📄 Analysis files saved to: docs/ui-captures/analysis/${this.sessionId}`,
      );
      console.log("\n💡 Next steps:");
      console.log(
        "   1. Review the analysis prompt in docs/ui-captures/analysis/",
      );
  ...
  async createLatestSymlink() {
    try {
      ...
      console.log(
        "⚠️  Could not create latest symlink (not critical):",
        error.message,
      );
    }
  }
  ...
  async analyzeOnly() {
    ...
    try {
      ...
      if (sessions.length === 0) {
        console.log(
          "❌ No capture sessions found. Run a capture session first.",
        );
        return;
      }
      ...
      console.log(
        `📄 Analysis prompt generated for ${manifest.captureCount} screenshots`,
      );
      console.log(
        `📁 Files saved to: docs/ui-captures/analysis/${latestSession}/`,
      );
    } catch (error) {
